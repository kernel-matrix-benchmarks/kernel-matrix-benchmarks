...
class HtoolProduct(BaseProduct):
    ...
    def __init__(
        self, *, kernel, dimension, normalize_rows=False, eta=10, epsilon=1e-3, maxblocksize=100, target_minclustersize=10, source_minclustersize=10, symmetry='N', UPLO='N',
    ):
        ...
        if kernel not in HtoolSupportedKernels:
            raise NotImplementedError(
                f"HtoolProduct doesn't support kernel {kernel}."
            )
        ...
        self.bench = HtoolBench.HtoolBenchmarkPCARegularClustering(
            dimension, HtoolSupportedKernels[kernel], "partialACA", self.symmetry, self.UPLO)
    ...
    def prepare_data(
        ...
        *,
        ...
        self.source_points = np.asfortranarray(
            source_points, dtype=np.double)
        self.NbCols = self.source_points.shape[0]
        self.target_points = (
            None
            if same_points
            else np.asfortranarray(target_points, dtype=np.double)
        )
        if same_points:
    ...
    def fit(self):
        ...
        if self.same_points:
            ...
            self.bench.build_clusters(
                self.NbRows, self.NbCols, self.target_points, self.source_points, self.target_minclustersize, self.source_minclustersize)
            self.bench.build_HMatrix(
                self.target_points, self.source_points, self.epsilon, self.eta, 0, 0, 100)
    ...
    def prepare_query(self, *, source_signal):
        ...
        if self.normalize_rows:
            ...
            signal_1 = np.concatenate(
                (self.source_signal, ones_column), axis=1)
