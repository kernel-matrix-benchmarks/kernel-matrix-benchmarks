...
class Milvus(BaseANN):
    ...
    def fit(self, X):
        ...
        self._milvus.create_table(
            {"table_name": self._table_name, "dimension": X.shape[1]}
        )
        vector_ids = [id for id in range(len(X))]
        self._milvus.insert(
            table_name=self._table_name, records=X.tolist(), ids=vector_ids
        )
        index_type = getattr(
            milvus.IndexType, self._index_type
        )  # a bit hacky but works
        self._milvus.create_index(
            self._table_name, {"index_type": index_type, "nlist": self._nlist}
        )
    ...
    def query(self, v, n):
        ...
        v = v.tolist()
        status, results = self._milvus.search(
            table_name=self._table_name, query_records=[v], top_k=n, nprobe=self._nprobe
        )
        if not results:
    ...
    def __str__(self):
        return "Milvus(index_type=%s, nlist=%d, nprobe=%d)" % (
            self._index_type,
            self._nlist,
            self._nprobe,
        )
